...
#### Scenario: Proxy an Anthropic Messages request
- **WHEN** a client sends a POST request to the gateway with path `/v1/messages` and a JSON body
- **THEN** the gateway forwards the request to the configured target API base URL with the same path, method, headers, and body, and returns the target's response to the client unchanged, using the Anthropic Messages parser for tracing

#### Scenario: Proxy an OpenAI Responses request
- **WHEN** a client sends a POST request to the gateway with path `/v1/responses` and a JSON body
- **THEN** the gateway forwards the request to the configured target API base URL with the same path, method, headers, and body, and returns the target's response to the client unchanged, using the OpenAI Responses parser for tracing
